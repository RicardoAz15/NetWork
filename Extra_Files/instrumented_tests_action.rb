  ...
  module Actions
    ...
    class InstrumentedTestsAction < Action
      ...
      def self.create_emulator(params)
        ...
        create_avd = ["echo no | #{params[:sdk_path]}/tools/bin/avdmanager", "create avd -f", avd_package, avd_name, avd_abi]
      ...
      def self.start_emulator(params)
        ...
        start_avd = ["#{params[:sdk_path]}/emulator/emulator.exe", "-avd #{params[:avd_name]}", "#{ui_args}", "-port #{params[:avd_port]}" ].join(" ")
	...
	sh("set ANDROID_SERIAL = #{@android_serial}")

        loop do

          boot_complete_cmd = "#{adb_path} shell getprop sys.boot_completed" 
      ...
      def self.stop_emulator(params)
        ...
        begin
          ...
          sh("#{params[:sdk_path]}/platform-tools/adb emu kill")
